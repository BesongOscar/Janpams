...
export async function updateActiveStreetFromLocation(
  ...
  useStreetSelectionStore.getState().setStreetSelectionDebug({
  ...
  useStreetSelectionStore.getState().setNearbyStreets(selectionResult.candidateStreets);

  if (!selectionResult.activeStreet) {
    useStreetSelectionStore.getState().setActiveStreetData(null);
    useStreetSelectionStore.getState().setResolvedStreetGeometry(null);
    useStreetSelectionStore.getState().setActiveStreetDirectionLock(null);
    useAddressStore.getState().setCalculatedAddress(null);
    useStreetSelectionStore.getState().setActiveStreet(null);
    return;
  }

  const activeStreetData = selectionResult.activeStreet;
  useStreetSelectionStore.getState().setActiveStreetData(activeStreetData);

  const streetKey = normalizeStreetKey(activeStreetData.name);
  const directionLock = await getDirectionLock(streetKey);
  useStreetSelectionStore.getState().setActiveStreetDirectionLock(directionLock);

  const street = activeStreetDataToStreet(activeStreetData);
  const resolved = resolveStreetGeometry(street, directionLock);
  useStreetSelectionStore.getState().setResolvedStreetGeometry(resolved);
  useStreetSelectionStore.getState().setActiveStreet(street);
  ...
  useAddressStore.getState().setCalculatedAddress(calculatedAddress ?? null);
}
...
export async function syncMapStoreFromResolveResult(
  ...
  useUserLocationStore.getState().setActiveLocation(activeLocation);
  useStreetSelectionStore.getState().setNearbyStreets(result.nearbyStreets);
  useStreetSelectionStore.getState().setStreetSelectionDebug({
  ...
  if (!result.activeStreet) {
    ...
    if (admin && (admin.city || admin.neighborhood || admin.region || admin.country)) {
      ...
      useAddressStore.getState().setCalculatedAddress(minimalAddress);
    } else {
      useAddressStore.getState().setCalculatedAddress(null);
    }
    return;
  ...
  useStreetSelectionStore.getState().setActiveStreetData(result.activeStreet);
  ...
  const calculatedAddress: AddressData | null =
    result.houseNumber != null && result.street
      ...
      : null;
  useAddressStore.getState().setCalculatedAddress(calculatedAddress);
}
...
export function useActiveStreet() {
  ...
  const streetSelectionDebug = useStreetSelectionStore((s) => s.streetSelectionDebug);
  ...
  const setNearbyStreets = useStreetSelectionStore((s) => s.setNearbyStreets);
  const setCalculatedAddress = useAddressStore((s) => s.setCalculatedAddress);
  const setStreetSelectionDebug = useStreetSelectionStore((s) => s.setStreetSelectionDebug);
